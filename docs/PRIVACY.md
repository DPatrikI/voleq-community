...
Speech probability is calculated with a bundled RNNoise model. Analysis state,
the learned background level, and delayed audio exist only in memory while
leveling is active and are reset when the audio route is rebuilt or processing
stops. Normal builds and the finished application perform no model download.
