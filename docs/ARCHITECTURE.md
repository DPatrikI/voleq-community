...
- The processor compares the aggregate callback's real input/output frame cadence, not only the tap's advertised rates. Equal callback periods indicate that tap drift compensation already synchronized the Bluetooth route and duplicate conversion must be bypassed; rate-proportional periods use Audio Converter Services with preallocated input and output FIFOs. The output FIFO pre-rolls briefly and writes only complete device periods. A full FIFO drops its oldest frame to recover at the live edge instead of accumulating latency, while sustained output underruns fail safely instead of emitting repeated partial periods.
