...
| Default lookahead at 16 / 44.1 / 48 kHz | 160 / 441 / 480 frames |
| Quiet-to-loud onset | First loud 10 ms no more than 1 dB above settled loud output |
...
The 10 ms figure is the DSP lookahead delay. A route that uses sample-rate conversion can add converter and device buffering latency outside the portable leveler.

### Listening gate

The following checks remain pending for this branch and must be completed before merge:
...
- [ ] Confirm that the initial 10 ms silence is not perceptible as a start or route-change defect.
