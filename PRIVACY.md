...
When the user chooses **Check for Updates**, or explicitly enables automatic checks, VolEq makes an unauthenticated request to `api.github.com` for the latest published release from the VolEq Community repository. Automatic checks run at most once in a rolling 24-hour period and only while VolEq is running. Opening **View Release** contacts `github.com`. GitHub receives ordinary network metadata such as the user's IP address. VolEq sends no audio, account identifier, user identifier, or usage telemetry, and it stores no GitHub response body, cookie, or credential. The setting can be changed in VolEq Settings.
