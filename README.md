...
- The Sennheiser HDB 630 and Apple AirPods Pro 2 have been physically validated in regular playback and call mode with their microphones active for the earlier speech-aware leveler. The owner accepted a physical mild-suppression test on 2026-08-02, but its device and microphone-mode matrix was not recorded and is not presented as per-device compatibility evidence. Deterministic tests cover 48↔44.1 kHz and 48→16 kHz paths, but live profile switching while VolEq remains active, broader headset compatibility, meeting apps, and long-running stability still need structured validation.
...
- The required 48 kHz stereo release benchmark currently measures 15.60% of one core on the development Mac, above the 5% target. This result must be improved or the target explicitly revisited before release; owner listening does not override the failed release gate.
